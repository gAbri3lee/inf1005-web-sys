...
if (isset($_SESSION['checkout_form_data']) && is_array($_SESSION['checkout_form_data'])) {
	foreach ($formData as $key => $_) {
		if (array_key_exists($key, $_SESSION['checkout_form_data'])) {
			$formData[$key] = trim((string)$_SESSION['checkout_form_data'][$key]);
		}
	}
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$errors) {
	...
	$cardDigits = checkout_digits_only($formData['card_number']);
	if ($cardDigits === '') {
	...
	$cvvDigits = checkout_digits_only($formData['cvv']);
	if ($cvvDigits === '') {
	...
	if ($errors) {
		...
		unset($_SESSION['pending_booking'], $_SESSION['checkout_form_data']);
			...
			<div class="content-card rooms-card reveal-up">
				...
				<?php if ($errors): ?>
					...
					<div class="d-grid d-md-flex justify-content-md-center gap-2 mt-3">
						<a class="btn btn-gold" href="<?php echo htmlspecialchars($checkoutErrorReturnPath, ENT_QUOTES, 'UTF-8'); ?>"><?php echo $checkoutErrorReturnPath === 'checkout.php' ? 'Back to Checkout' : 'Back to Rooms'; ?></a>
					</div>
